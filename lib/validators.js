...
export function validateGenerateImageParameters({
  prompt = null,
  imageModel = null,
  dimesions = null,
}) {
  ...
  if (
    dimesions &&
    typeof dimesions != "string" &&
    !dimensionsObject[imageModel].includes(dimesions)
  ) {
    throw new Error(
      `Invalid 'dimesions' parameter. It should be a ${dimensionsObject[
        imageModel
      ].join(", ")}.`
    );
  }
