...
const requestParametersSchema = Joi.object({
  prompt: Joi.alternatives().try(Joi.string(), Joi.array()).allow(null),
  imageOptions: Joi.object().allow(null),
  requestOptions: requestOptionsSchema.allow(null),
}).custom((value, helpers) => {
  if (value.prompt === null && Object.keys(value.requestOptions).length === 0) {
    return helpers.error("prompt or requestOptions.messages is required");
  }
});
